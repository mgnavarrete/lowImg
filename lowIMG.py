...
for root, dirs, files in os.walk(lowImg):
    for file in tqdm(files, desc="Subiendo archivos a S3"):
        local_path = f"{task_name}/obj_train_data/{task_name.split('-')[0]}/{task_name.split('-')[1]}/images/{file}"
       
        s3_full_key = f"{task_name.split('-')[0]}/{task_name.split('-')[1]}/img_mala_calidad/{file}"
